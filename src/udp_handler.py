...
#ROI_PATH = "/root/smart-city-compss/data/barcelona/encants/roi/encants.json"
...
def run_udp(
    ...
    for edge_device in edge_ips:
        ...
        if save_plot or view_plot:
            ...
            print(f'xxxxxx {CAM_WIDTH} {CAM_HEIGHT}')
        ...
        # We get the ROI with getRoi from utils
        ...
        while frame_idx < NUM_ITERS:                            
            ...
            save_plot = True ###################### TODO CAMBIAR Y ARREGLAR
            if save_plot:
                ret, frame = cap.read()
                if not ret:
                    break
            ...
            view_plot = False ############################ TODO CAMBIAR/ARREGLAR
            if view_plot:
                if not save_plot:
                    ret, frame = cap.read()
                    if not ret:
            ...
            # Save frames into runs/datetimeofrun
            run_dir = os.path.join("runs", datetime.now().strftime("%Y-%m-%d_%H-%M-%S"))
            os.makedirs(run_dir, exist_ok=True)

            # Save frame as JPG
            frame_filename = os.path.join(run_dir, f"{frameId}.jpg")
            cv2.imwrite(frame_filename, online_im)
        ...
        if alerts:
            ...
            for alert in alertInfo:
                with open(alarm_file, 'w') as f:
                    f.writelines(alert.saveAlert())
